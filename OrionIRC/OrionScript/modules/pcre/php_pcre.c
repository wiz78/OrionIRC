...
/* $Id: php_pcre.c,v 1.1 2005/01/22 15:07:28 wiz Exp $ */
...
#include "php.h"
#include "php_pcre.h"
#include "modules/base/php_smart_str.h"
#include "modules/base/php_string.h"
...
static int preg_do_repl_func(zval *function, char *subject, int *offsets, int count, char **result TSRMLS_DC)
{
	...
	if (call_user_function_ex(EG(function_table), NULL, function, &retval_ptr, 1, args, 0, NULL TSRMLS_CC) == SUCCESS && retval_ptr) {
		...
		*result = estrndup(Z_STRVAL_P(retval_ptr), Z_STRLEN_P(retval_ptr));
		...
		zend_error(E_WARNING, "Unable to call custom replacement function");
							  ...
							  int *result_len, int limit TSRMLS_DC)
{
	...
	if (is_callable_replace) {
		if (eval) {
			zend_error(E_WARNING, "/e modifier cannot be used with replacement callback");
			return NULL;
		}
	} else {
...
static void preg_replace_impl(INTERNAL_FUNCTION_PARAMETERS, zend_bool is_callable_replace)
{
	...
	if (!is_callable_replace && Z_TYPE_PP(replace) == IS_ARRAY && Z_TYPE_PP(regex) != IS_ARRAY) {
		zend_error(E_WARNING, "Parameter mismatch, pattern is a string while replacement in an array.");
		RETURN_FALSE;
	}
	...
	if (is_callable_replace) {
		if (!zend_is_callable(*replace, 0, &callback_name)) {
			zend_error(E_WARNING, "requires argument 2, '%s', to be a valid callback", callback_name);
			efree(callback_name);
		...
		if (count > 0) {
			...
			if (g_notempty != 0 && start_offset < Z_STRLEN_PP(subject)) {
				if (coptions & PCRE_UTF8) {
					...
					if (count < 1) {
						zend_error(E_NOTICE, "Unknown error");
						offsets[0] = start_offset;
						offsets[1] = start_offset + 1;
					}
				} else {
