...
/* $Id: scanf.c,v 1.1 2005/01/22 15:07:28 wiz Exp $ */
...
#define STATIC_LIST_SIZE 16
	...
	notXpg:
		...
		if (gotXpg) {
			mixedXPG:
			  zend_error(E_WARNING, "%s", "cannot mix \"%\" and \"%n$\" conversion specifiers");
			goto error;
		}
	...
	xpgCheckDone:
		...
		switch (*ch) {
			...
			case '[':
				...
				break;
			badSet:
				zend_error(E_WARNING, "Unmatched [ in format string");
				goto error;
			default:
				{
				zend_error(E_WARNING, "Bad scan conversion character \"%c\"", *ch);
				goto error;
				}
		}
	...
	for (i = 0; i < numVars; i++) {
		if (nassign[i] > 1) {
			zend_error(E_WARNING, "%s", "Variable is assigned by multiple \"%n$\" conversion specifiers");
			goto error;
			 ...
			 */
			zend_error(E_WARNING, "Variable is not assigned by any conversion specifiers");
			goto error;
		}
	}
...
badIndex:
	if (gotXpg) {
		zend_error(E_WARNING, "%s", "\"%n$\" argument index out of range");
	} else {
		zend_error(E_WARNING, "Different numbers of variable names and field specifiers");
	}
	...
	if (numVars) {
		for (i = varStart;i < argCount;i++){
			if ( ! PZVAL_IS_REF( *args[ i ] ) ) {	
				zend_error(E_WARNING, "Parameter %d must be passed by reference", i);
				scan_set_error_return(numVars, return_value);
