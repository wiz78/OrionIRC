...
/* $Id: var.c,v 1.1 2005/01/22 00:27:37 wiz Exp $ */
	...
	if (count > 0) {
		...
		for (;; zend_hash_move_forward_ex(HASH_OF(retval_ptr), &pos)) {
			...
			if (Z_TYPE_PP(name) != IS_STRING) {
				zend_error(E_NOTICE, "__sleep should return an array only "
			...
			if (zend_hash_find(Z_OBJPROP_PP(struc), Z_STRVAL_PP(name), 
				...
				if (ce) {
					...
					do {
						...
						efree(prot_name);
						zend_error(E_NOTICE, "\"%s\" returned as member variable from __sleep() but does not exist", Z_STRVAL_PP(name));
						php_var_serialize_string(buf, Z_STRVAL_PP(name), Z_STRLEN_PP(name));
...
static void php_var_serialize_intern(smart_str *buf, zval **struc, HashTable *var_hash TSRMLS_DC)
{
	...
	switch (Z_TYPE_PP(struc)) {
		...
		case IS_DOUBLE: {
				...
				slen = spprintf(&s, 0, "d:%.*G;", 100, Z_DVAL_PP(struc));
...
ZEND_FUNCTION(unserialize)
{
	...
	if (Z_TYPE_PP(buf) == IS_STRING) {
		...
		if (!php_var_unserialize(&return_value, &p, p + Z_STRLEN_PP(buf),  &var_hash TSRMLS_CC)) {
			...
			zend_error(E_NOTICE, "Error at offset %ld of %d bytes", (long)((char*)p - Z_STRVAL_PP(buf)), Z_STRLEN_PP(buf));
			RETURN_FALSE;
		}
		PHP_VAR_UNSERIALIZE_DESTROY(var_hash);
	} else {
		zend_error(E_NOTICE, "Argument is not a string");
		RETURN_FALSE;
	}
}
