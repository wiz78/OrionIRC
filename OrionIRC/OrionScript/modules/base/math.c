...
/* $Id: math.c,v 1.1 2005/01/22 15:07:28 wiz Exp $ */
	...
	switch (ZEND_NUM_ARGS()) {
		...
		case 2:
			...
			if (Z_DVAL_PP(base) <= 0.0) {
				zend_error(E_WARNING, "base must be greater than 0");				
				RETURN_FALSE;
			}
...
_php_math_basetolong(zval *arg, int base) {
	...
	for (i = Z_STRLEN_P(arg); i > 0; i--) {
...

		{
			...
			zend_error(E_WARNING, "Number '%s' is too big to fit in long", s);
			return LONG_MAX;
		}
	}
...
_php_math_zvaltobase(zval *arg, int base TSRMLS_DC)
{
	...
	if (Z_TYPE_P(arg) == IS_DOUBLE) {
		...
		if (fvalue == HUGE_VAL || fvalue == -HUGE_VAL) {
			zend_error(E_WARNING, "Number too large");
			return empty_string;
		}
...
PHP_FUNCTION(base_convert)
{
	...
	if (Z_LVAL_PP(frombase) < 2 || Z_LVAL_PP(frombase) > 36) {
		zend_error(E_WARNING, "Invalid `from base' (%ld)", Z_LVAL_PP(frombase));
		RETURN_FALSE;
	}
	if (Z_LVAL_PP(tobase) < 2 || Z_LVAL_PP(tobase) > 36) {
		zend_error(E_WARNING, "Invalid `to base' (%ld)", Z_LVAL_PP(tobase));
		RETURN_FALSE;
	}
