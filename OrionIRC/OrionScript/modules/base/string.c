...
/* $Id: string.c,v 1.2 2005/01/22 15:07:28 wiz Exp $ */
	...
	for (end = input+len; input < end; input++) {
		...
		if ((input+3 < end) && input[1] == '.' && input[2] == '.' 
			...
			if (end-len >= input) { /* there was no 'left' char */
				zend_error(E_WARNING, "Invalid '..'-range, no character to the left of '..'.");
				result = FAILURE;
				continue;
			}
			if (input+2 >= end) { /* there is no 'right' char */
				zend_error(E_WARNING, "Invalid '..'-range, no character to the right of '..'.");
				result = FAILURE;
				continue;
			}
			if (input[-1] > input[2]) { /* wrong order */
				zend_error(E_WARNING, "Invalid '..'-range, '..'-range needs to be incrementing.");
				result = FAILURE;
				continue;
			} 
			/* FIXME: better error (a..b..c is the only left possibility?) */
			zend_error(E_WARNING, "Invalid '..'-range.");
			result = FAILURE;
			continue;
		} else {
			mask[c]=1;
...
ZEND_FUNCTION(wordwrap)
{
	...
	if (linelength == 0 && docut) {
		zend_error(E_WARNING, "Can't force cut when width is zero.");
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(implode)
{
	...
	if (argc == 1) {
		if (Z_TYPE_PP(arg1) != IS_ARRAY) {
			zend_error(E_WARNING, "Argument to implode must be an array.");
			return;
		}
...
#define _IMPL_EMPTY ""
		...
		if (Z_TYPE_PP(arg1) == IS_ARRAY) {
			...
			zend_error(E_WARNING, "Bad arguments.");
			return;
		}
	}
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		switch (Z_TYPE_P(needle)) {
			...
			case IS_DOUBLE:
				...
				break;
			default:
				zend_error(E_WARNING, "needle is not a string or an integer.");
				efree(haystack_dup);
				RETURN_FALSE;
				break;
...
ZEND_FUNCTION(chunk_split) 
{
	...
	if (chunklen <= 0) {
		zend_error(E_WARNING, "Chunk length should be greater than zero.");
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(substr_replace)
{
	...
	if (Z_TYPE_PP(str) == IS_STRING) {
		if (
			...
			zend_error(E_WARNING, "'from' and 'len' should be of same type - numerical or array ");
		...
		if (argc == 4 && Z_TYPE_PP(from) == IS_ARRAY) {
			if (zend_hash_num_elements(Z_ARRVAL_PP(from)) != zend_hash_num_elements(Z_ARRVAL_PP(len))) {
				zend_error(E_WARNING, "'from' and 'len' should have the same number of elements");
				RETURN_STRINGL(Z_STRVAL_PP(str), Z_STRLEN_PP(str), 1);		
			}
		}
	}
	...
	if (Z_TYPE_PP(str) != IS_ARRAY) {
		if (Z_TYPE_PP(from) != IS_ARRAY) {
			...
			zend_error(E_WARNING, "Functionality of 'from' and 'len' as arrays is not implemented.");
			RETURN_STRINGL(Z_STRVAL_PP(str), Z_STRLEN_PP(str), 1);	
		}
...
{								
	...
	if (ac == 2 && Z_TYPE_PP(from) != IS_ARRAY) {
		zend_error(E_WARNING, "The second argument is not an array.");
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(str_repeat)
{
	...
	if (Z_LVAL_PP(mult) < 0) {
		zend_error(E_WARNING, "Second argument has to be greater than or equal to 0.");
		return;
	}
	...
	if (result_len < 1 || result_len > 2147483647) {
		zend_error(E_WARNING, "You may not create strings longer then 2147483647 bytes");
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(count_chars)
{
	...
	if (ac == 2) {
		...
		if (mymode < 0 || mymode > 4) {
			zend_error(E_WARNING, "Unknown mode.");
			RETURN_FALSE;
		}
	}
...
ZEND_FUNCTION(substr_count)
{
	...
	if (Z_STRLEN_PP(needle) == 0) {
		zend_error(E_WARNING, "Empty substring.");
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(str_pad)
{
	...
	if (ZEND_NUM_ARGS() > 2) {
		...
		if (Z_STRLEN_PP(pad_string) == 0) {
			zend_error(E_WARNING, "Padding string cannot be empty.");
			return;
		}
		...
		if (ZEND_NUM_ARGS() > 3) {
			...
			if (pad_type_val < STR_PAD_LEFT || pad_type_val > STR_PAD_BOTH) {
				zend_error(E_WARNING, "Padding type has to be STR_PAD_LEFT, STR_PAD_RIGHT, or STR_PAD_BOTH.");
				return;
			}
		}
	}
...
ZEND_FUNCTION(str_word_count)
{
	...
	if (n_args == 2) {
		...
		if (type != 1 && type != 2) {
			zend_error(E_WARNING, "The specified format parameter, '%d' is invalid.", type);
			RETURN_FALSE;
		}
	}
...
ZEND_FUNCTION(str_split)
{
	...
	if (split_length <= 0) {
		zend_error(E_WARNING, "The length of each segment must be greater than zero.");
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(strpbrk)
{
	...
	if (!char_list_len) {
		zend_error(E_WARNING, "The character list cannot be empty.");
		RETURN_FALSE;	
	}
...
ZEND_FUNCTION(substr_compare)
{
	...
	if (len && offset >= s1_len) {
		zend_error(E_WARNING, "The start position cannot exceed initial string length.");
		RETURN_FALSE;
	}
