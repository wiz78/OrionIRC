...
/* $Id: array.c,v 1.1 2005/01/22 15:07:28 wiz Exp $ */
...
static void php_natsort(INTERNAL_FUNCTION_PARAMETERS, int fold_case)
{
	...
	target_hash = HASH_OF(*array);
	if (!target_hash) {
		zend_error(E_WARNING, "The argument should be an array");
		return;
	}
...
#define PHP_ARRAY_CMP_FUNC_CHECK(func_name)	\
	if (!zend_is_callable(*func_name, 0, NULL)) {	\
		zend_error(E_WARNING, "Invalid comparison function.");	\
...
PHP_FUNCTION(end)
{
	...
	target_hash = HASH_OF(*array);
	if (!target_hash) {
		zend_error(E_WARNING, "Passed variable is not an array or object");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(min)
{
	...
	if (argc<=0) {
		zend_error(E_WARNING, "Atleast one value should be passed");
		RETURN_NULL();
	}
	...
	if (argc == 1) {
		...
		if (zend_hash_minmax(Z_ARRVAL_PP(arr), array_data_compare, 0, (void **) &result TSRMLS_CC) == SUCCESS) {
			*return_value = **result;
			...
			zend_error(E_WARNING, "Array must contain atleast one element");
			RETURN_FALSE;
		}
	} else {
		...
		if (recursive && Z_TYPE_PP(args[0]) == IS_ARRAY) {
			...
			if (zend_call_function(&fci, &BG(array_walk_fci_cache) TSRMLS_CC) == SUCCESS) {
				...
				if (zend_is_callable(*BG(array_walk_func_name), 0, &func_name)) {
					zend_error(E_WARNING, "Unable to call %s()", func_name);
				} else {
					zend_error(E_WARNING, "Unable to call %s() - function does not exist", func_name);
				}
...
PHP_FUNCTION(array_walk)
{
	...
	if (Z_TYPE_PP(BG(array_walk_func_name)) != IS_ARRAY && 
		...
		zend_error(E_WARNING, "Wrong syntax for function name");
		BG(array_walk_func_name) = old_walk_func_name;
		RETURN_FALSE;
	}
...
static void php_search_array(INTERNAL_FUNCTION_PARAMETERS, int behavior)
{
	...
	if (Z_TYPE_PP(array) != IS_ARRAY) {
		zend_error(E_WARNING, "Wrong datatype for second argument");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(extract)
{
	...
	switch (ZEND_NUM_ARGS()) {
		...
		case 2:
			...
			if (extract_type > EXTR_SKIP && extract_type <= EXTR_PREFIX_IF_EXISTS) {
				zend_error(E_WARNING, "Prefix expected to be specified");
				return;
			}
			break;
	...
	if (extract_type < EXTR_OVERWRITE || extract_type > EXTR_IF_EXISTS) {
		zend_error(E_WARNING, "Unknown extract type");
		return;
	}
	
	if (Z_TYPE_PP(var_array) != IS_ARRAY) {
		zend_error(E_WARNING, "First argument should be an array");
		return;
	}
...
PHP_FUNCTION(array_fill)
{
	...
	switch (Z_TYPE_PP(start_key)) {
		...
		case IS_DOUBLE:
			...
			break;
		default:
			zend_error(E_WARNING, "Wrong data type for start key");
			RETURN_FALSE;
			break;
	}	
	...
	i = Z_LVAL_PP(num) - 1;	
	if (i < 0) {
		zend_error(E_WARNING, "Number of elements must be positive");
		RETURN_FALSE;
	}
	newval = *val;
	while (i--) {
...
err:
	if (err) {
		zend_error(E_WARNING, "step exceeds the specified range");
...
PHP_FUNCTION(array_unshift)
{
	...
	stack = *args[0];
	if (Z_TYPE_P(stack) != IS_ARRAY) {
		zend_error(E_WARNING, "The first argument should be an array");
		efree(args);
		RETURN_FALSE;
	}
...
static void php_array_merge_wrapper(INTERNAL_FUNCTION_PARAMETERS, int recursive)
{
	...
	for (i = 0; i < argc; i++) {
		if (Z_TYPE_PP(args[i]) != IS_ARRAY) {
			zend_error(E_WARNING, "Argument #%d is not an array", i+1);
			params_ok = 0;
		}
	}
	if (params_ok == 0) {
...
PHP_FUNCTION(array_pad)
{
	...
	*return_value = **input;
	...
	if(num_pads > 1048576) {
		zend_error(E_WARNING, "You may only pad up to 1048576 elements at a time");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(array_flip)
{
	...
	while (zend_hash_get_current_data_ex(target_hash, (void **)&entry, &pos) == SUCCESS) {
		...
		if (Z_TYPE_PP(entry) == IS_LONG) {
			...
			zend_error(E_WARNING, "Can only flip STRING and INTEGER values!");
		}
	...
	if (behavior == INTERSECT_NORMAL) {
		...
		if (data_compare_type == INTERSECT_COMP_DATA_INTERNAL) {
			...
			if (!zend_is_callable(*args[arr_argc], 0, &callback_name)) {
				zend_error(E_WARNING, "Not a valid callback %s", callback_name);
				efree(callback_name);
				efree(args);
				return;
			}
			...
			zend_error(E_WARNING, "data_compare_type is %d. This should never happen. Please report as a bug", data_compare_type);
			return;
		}
		...
		if (data_compare_type == INTERSECT_COMP_DATA_INTERNAL
			...
			zend_error(E_WARNING, "data_compare_type is %d. key_compare_type is %d. This should never happen. Please report as a bug.", data_compare_type, key_compare_type);
			return;
		}		
	} else {
		zend_error(E_WARNING, "behavior is %d. This should never happen. Please report as a bug", behavior);
		return;
	}
...
PHP_FUNCTION(array_multisort)
{
			...
			switch (Z_LVAL_PP(args[i])) {
				...
				case SORT_DESC:
					...
					if (parse_state[MULTISORT_ORDER] == 1) {
						...
						zend_error(E_WARNING, "Argument #%d is expected to be an array or sorting flag that has not already been specified", i+1);
						MULTISORT_ABORT;
					}
					break;
				...
				default:
					zend_error(E_WARNING, "Argument #%d is an unknown sort flag", i + 1);
					MULTISORT_ABORT;
					break;

			}
		} else {
			zend_error(E_WARNING, "Argument #%d is expected to be an array or a sort flag", i + 1);
			MULTISORT_ABORT;
		}
	}
	...
	for (i = 0; i < num_arrays; i++) {
		if (zend_hash_num_elements(Z_ARRVAL_PP(arrays[i])) != array_size) {
			zend_error(E_WARNING, "Array sizes are inconsistent");
			MULTISORT_ABORT;
		}
	}
...
PHP_FUNCTION(array_rand)
{
	...
	if (Z_TYPE_PP(input) != IS_ARRAY) {
		zend_error(E_WARNING, "First argument has to be an array");
		return;
	}
	...
	if (ZEND_NUM_ARGS() > 1) {
		...
		if (num_req_val <= 0 || num_req_val > num_avail) {
			zend_error(E_WARNING, "Second argument has to be between 1 and the number of elements in the array");
			return;
		}
	} else
		num_req_val = 1;
...
PHP_FUNCTION(array_reduce)
{
	...
	if (!zend_is_callable(*callback, 0, &callback_name)) {
		zend_error(E_WARNING, "The second argument, '%s', should be a valid callback", callback_name);
		efree(callback_name);
		return;
	}
	efree(callback_name);
	...
	while (zend_hash_get_current_data_ex(htbl, (void **)&operand, &pos) == SUCCESS) {
		if (result) {
			...
			if (zend_call_function(&fci, &fci_cache TSRMLS_CC) == SUCCESS && retval) {
				...
				zend_error(E_WARNING, "An error occurred while invoking the reduction callback");
				return;
			}
		} else {
			result = *operand;
	...
	for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_PP(input), &pos);
		...
		if (callback) {
			...
			if (zend_call_function(&fci, &fci_cache TSRMLS_CC) == SUCCESS && retval) {
				...
				zend_error(E_WARNING, "An error occurred while invoking the filter callback");
				return;
			}
...
PHP_FUNCTION(array_map)
{
	...
	if (Z_TYPE_P(callback) != IS_NULL) {
		if (!zend_is_callable(callback, 0, &callback_name)) {
			zend_error(E_WARNING, "The first argument, '%s', should be either NULL or a valid callback", callback_name);
			efree(callback_name);
			efree(pargs);
			return;
		}
	...
	for (i = 1; i < ZEND_NUM_ARGS(); i++) {
		if (Z_TYPE_PP(pargs[i]) != IS_ARRAY) {
			zend_error(E_WARNING, "Argument #%d should be an array", i + 1);
			efree(pargs);
			efree(args);
			efree(array_len);
			efree(array_pos);
	...
	for (k = 0; k < maxlen; k++) {
		...
		if (Z_TYPE_P(callback) != IS_NULL) {
			...
			if (!zend_call_function(&fci, &fci_cache TSRMLS_CC) == SUCCESS && result) {
				zend_error(E_WARNING, "An error occurred while invoking the map callback");
				efree(array_len);
				efree(args);
				efree(array_pos);
				zval_dtor(return_value);
...
PHP_FUNCTION(array_key_exists)
{
	...
	if (Z_TYPE_PP(array) != IS_ARRAY && Z_TYPE_PP(array) != IS_OBJECT) {
		zend_error(E_WARNING, "The second argument should be either an array or an object");
		RETURN_FALSE;
	}
	...
	switch (Z_TYPE_PP(key)) {
		...
		default:
			zend_error(E_WARNING, "The first argument should be either a string or an integer");
			RETURN_FALSE;
	}
...
PHP_FUNCTION(array_chunk)
{
	...
	if (size < 1) {
		zend_error(E_WARNING, "Size parameter expected to be greater than 0");
		return;
	}
...
PHP_FUNCTION(array_combine)
{
	...
	if (zend_hash_num_elements(Z_ARRVAL_P(keys)) == 0 || zend_hash_num_elements(Z_ARRVAL_P(values)) == 0) {
		zend_error(E_WARNING, "Both parameters should have number of elements at least 0");
		RETURN_FALSE;
	}

		
	if (zend_hash_num_elements(Z_ARRVAL_P(keys)) != zend_hash_num_elements(Z_ARRVAL_P(values))) {
		zend_error(E_WARNING, "Both parameters should have equal number of elements");
		RETURN_FALSE;
	}
