...
/* $Id: type.c,v 1.1 2005/01/22 00:27:37 wiz Exp $ */
...
ZEND_FUNCTION(settype)
{
	...
	if (!strcasecmp(new_type, "integer")) {
		...
		zend_error(E_WARNING, "Cannot convert to resource type");
		RETURN_FALSE;
	} else {
		zend_error(E_WARNING, "Invalid type");
		RETURN_FALSE;
	}
	RETVAL_TRUE;
}
...
static void php_is_type(INTERNAL_FUNCTION_PARAMETERS, int type)
{
	...
	if (ZEND_NUM_ARGS() != 1 || zend_get_parameters_ex(1, &arg) == FAILURE) {
		zend_error(E_WARNING, "Only one argument expected");
		RETURN_FALSE;
	}
