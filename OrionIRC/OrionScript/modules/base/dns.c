...
/* $Id: dns.c,v 1.1 2005/01/22 15:07:28 wiz Exp $ */
...
PHP_FUNCTION(gethostbyaddr)
{
	...
	if (addr == NULL) {
		...
		zend_error(E_WARNING, "Address is not a valid IPv4 or IPv6 address");
#else
		zend_error(E_WARNING, "Address is not in a.b.c.d form");
#endif
		RETVAL_FALSE;
	} else {
	...
	switch (ZEND_NUM_ARGS()) {
		...
		case 2:
			...
			if (!strcasecmp("A", Z_STRVAL_PP(arg2))) type = T_A;
				...
				zend_error(E_WARNING, "Type '%s' not supported", Z_STRVAL_PP(arg2));
				RETURN_FALSE;
			}
			break;
...
PHP_FUNCTION(dns_get_record)
{
	...
	if (type_param&~PHP_DNS_ALL && type_param!=PHP_DNS_ANY) {
		zend_error(E_WARNING, "Type '%d' not supported", type_param);
		RETURN_FALSE;
	}
	...
	for(type = (type_param==PHP_DNS_ANY ? (PHP_DNS_NUM_TYPES + 1) : 0); type < (addtl_recs ? (PHP_DNS_NUM_TYPES + 2) : PHP_DNS_NUM_TYPES) || first_query; type++)
	{
		...
		if (type_to_fetch) {
			...
			if (n<0) {
				zend_error(E_WARNING, "res_nmkquery() failed");
				zval_dtor(return_value);
				RETURN_FALSE;
			}
			n = res_nsend(&res, buf.qb2, n, answer.qb2, sizeof answer);
			if (n<0) {
				zend_error(E_WARNING, "res_nsend() failed");
				zval_dtor(return_value);
				RETURN_FALSE;
			}
			...
			while (qd-- > 0) {
				...
				if (n < 0) {
					zend_error(E_WARNING, "Unable to parse DNS data received");
					zval_dtor(return_value);
					RETURN_FALSE;
				}
				cp += n + QFIXEDSZ;
