...
/* $Id: datetime.c,v 1.1 2005/01/22 15:07:28 wiz Exp $ */
...
#ifdef PHP_WIN32
	if (t - chgsecs < 0) {
		zend_error(E_WARNING, "Windows does not support negative values for this function");
		RETURN_LONG(-1);
	}
#endif
...
static void php_date(INTERNAL_FUNCTION_PARAMETERS, int gm)
{
...
#ifdef PHP_WIN32
		if (the_time < 0) {
			zend_error(E_WARNING, "Windows does not support dates prior to midnight (00:00:00), January 1, 1970");
			RETURN_FALSE;
		}
#endif
		break;
	...
	if (!ta) {			/* that really shouldn't happen... */
		zend_error( E_WARNING, "Unexpected error");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(idate)
{
	...
	if (Z_STRLEN_PP(format) != 1) {
		zend_error(E_WARNING, "idate format is one char");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(localtime)
{
	...
	if (NULL == (ta = php_localtime_r(&timestamp, &tmbuf))) {
		zend_error(E_WARNING, "Invalid local time");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(getdate)
{
	...
	if (!ta) {
		zend_error(E_WARNING, "Cannot perform date calculation");
		return;
	}
	array_init(return_value);
...
PHP_FUNCTION(strtotime)
{
	...
	if (!Z_STRLEN_PP(z_time)) {
		zend_error(E_WARNING, "Called with an empty time parameter.");
		RETURN_LONG(-1);
	}
	if (argc == 2) {
